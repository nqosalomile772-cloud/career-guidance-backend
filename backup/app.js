...
// Initialize Firebase Admin for both local and Functions environments
const serviceAccountPath = path.join(__dirname, 'serviceAccountKey.json');
if (!admin.apps.length) {
  try {
    if (process.env.FIREBASE_FUNCTIONS) {
      admin.initializeApp();
      console.log('✅ Firebase Admin initialized (Functions environment)');
      ...
      // Fallback to default credentials (useful if running with GOOGLE_APPLICATION_CREDENTIALS)
      admin.initializeApp();
...
// --- Routes (kept same as original index.js) ---
...
app.post('/student/apply', checkRole(['student']), async (req, res) => {
  ...
  try {
    ...
    if (!inst.exists) return res.status(404).send('Institution not found');
    const course = inst.data().faculties.flatMap(f => f.courses || []).find(c => c.name === courseId);
